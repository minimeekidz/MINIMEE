...
# 由 magenta sheet 抽幀（`scripts/extract-pets.mjs`）

上載嘅 sprite sheet 用洋紅（#FF00FF）做背景色 —— 呢個係做 sprite 嘅
標準去背色，因為藝術本身唔會用到呢隻色。跑：

```sh
node scripts/extract-pets.mjs <sheet.png> <輸出資料夾> <檔名前綴>
```

會自動：摳走洋紅、消除邊緣粉紅光暈、按空白直行分幀、
每幀置中放入正方形畫布並留 8% 空白、輸出 400×400 webp。

## ✅ 呢個做法救返咗被切嘅頭

`MM_PET_*` 嗰批頭頂被切平（最頂三行等闊）。magenta sheet 嗰批
內容由第 165 行先開始 —— 有足足 165px 頭部空間，完全冇切。
所以**用 sheet 抽出嚟嗰啲先係好嘅**。

## ⚠️ 三張 sheet 抽唔到

`(2)`、`(7)`、`(19)` —— 其中 `(19)` 係黑底唔係洋紅底，另外兩張
啲角色貼到太貼冇空白直行可以分。重出呢三張嘅話：洋紅底、
每隻之間留至少 20px 純背景。

## 目前有嘅

`bao-hamster/` —— 100 幀，全部係**包子倉鼠一隻**（表情同角度變化）。
其餘 11 隻仲未有 magenta sheet 版本。
