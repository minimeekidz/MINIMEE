...
create table if not exists public.notifications (
  ...
  parent_id uuid not null references auth.users(id) on delete cascade,
  source_order_id uuid not null unique references public.billing_orders(id) on delete restrict,
  source_event_id text not null unique references public.stripe_events(event_id) on delete restrict,
  notification_type text not null,
  title text not null,
  body text not null,
  channels text[] not null default array['in_app', 'anonymous_email']::text[],
...
grant all on public.billing_orders, public.subscriptions, public.theme_entitlements, public.stripe_events, public.notifications to service_role;
...
revoke all on function public.finalize_verified_checkout(
  text, text, text, text, uuid, uuid, uuid, text, integer, text, text
) from public, anon, authenticated;
grant execute on function public.finalize_verified_checkout(
  text, text, text, text, uuid, uuid, uuid, text, integer, text, text
) to service_role;
