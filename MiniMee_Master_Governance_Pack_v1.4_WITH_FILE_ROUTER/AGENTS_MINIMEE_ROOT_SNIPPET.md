...
2. Then read `SKILL_v1.3.md`.
3. Use the Router to load every mandatory source-of-truth file for the requested change.
4. For a new feature/event/item/theme, run `FEATURE_EVENT_GOVERNANCE.md` intake before implementation.
5. Ask only missing critical questions; do not silently invent game logic.
6. Do not implement from chat memory, screenshots, or prototype images when a canonical registry, scene manifest, UI registry, character Bible, or content source exists.
