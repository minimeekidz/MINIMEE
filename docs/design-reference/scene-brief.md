...
## ⚠️ 三張戶外圖仲係舊 set（2026-08-21 查證）

Em：「小遊戲背景圖錯了，用了舊版（即有角色係背景果set）」——**你講得啱**，
但成因同你估嘅唔同，所以要講清楚：

| 場景 | `public/assets/world/` 入面嗰張 | 同你上載嗰張 |
|---|---|---|
| 小鎮中心 日／夜 | 有角色畫死咗喺背景（柴犬、刺蝟、企鵝、貓、兔） | **一模一樣**（md5 相同） |
| 小鎮廣場 日／夜 | 冇角色，但冇舞台 | **一模一樣** |
| 碼頭市集 日／夜 | 有角色畫死咗（熊、貓、企鵝、浣熊） | **一模一樣** |
| 散步公園、小屋區入口、其餘九個室內 | 新版 | 已更新（有 `-wide` 橫向版） |

即係話：`world/` 入面嗰三張，同你 `uploads/場景-地圖/` 入面嗰三張係**同一個檔**。
新版嘅三張**由頭到尾未入過 repo** —— 只喺對話入面出現過，冇上載過。
分辨方法好簡單：**你逐張上載過嘅場景都有一個 `-wide` 橫向版，呢三張冇。**

### 已經做咗嘅

- 三個場景嘅 hotspot 同行走路線**全部重畫過**，跟返實際出貨嗰張圖量度，
  唔再係跟一張只存在過喺對話入面嘅圖。之前啲門口係企喺屋頂同海面上。
- 舊到唔能再舊嗰 16 張場景圖（`town-morning-v2`、`dock-town-dusk`、
  `amusement-park`、`mushroom-city-morning` 嗰個世代）同埋淨係服務佢哋嘅
  `scripts/make-night.mjs` **已經刪晒**。

### 要你補嘅（三張 × 日夜 × 直橫 = 12 個檔）

`小鎮中心`、`小鎮廣場`、`碼頭市集`，日更同夜更，直度同橫度。
**背景唔好畫角色** —— 遊戲會自己喺上面畫寵物同 NPC，畫死咗會變兩隻。
小鎮廣場嗰張**要有個舞台**（圓形木台就得），而家個舞台掣係企喺空地上面。

檔名照舊放 `public/assets/uploads/場景-地圖/`，我會接。
