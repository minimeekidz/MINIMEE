...
**好感度 (`src/lib/petFriends.ts`)** — twelve levels, 30 points apart, and
the scoring is deliberately tiny:

- **Turning up is worth 1 point, once a day, per pet** — not one point per
  action. Paying per action made it homework: a child worked down the list to
  farm the number instead of doing what they felt like. Actions now carry no
  value and no cap at all, so tapping 攬一攬 ten times is something you do
  because you want to.
- **Answering that pet's word question is worth 1 more, for the first two
  pets a day across the whole town** (`DAILY_QUIZ_SLOTS`). The scarcity is
  the design: twelve pets, two slots, so a child has to choose who they are
  actually befriending rather than spreading thin over all of them.
- **A wrong answer costs nothing.** Two tries; miss both and the pet gives
  the answer and says try again tomorrow — and **the slot is not spent**, so
  they can go and find another friend. Losing the day's chance for guessing
  is how you teach a five-year-old to stop guessing.
- **Friendship is per pet.** The panel is **keyed by pet id**: unkeyed, React
  reused one component instance and its running total followed the child from
  animal to animal — invisible whenever two pets sat on the same score, so
  true of every pair at the start.

The arithmetic, and why it fits a year's subscription:

| | pace |
|---|---|
| a pet you only visit | 1 point/day → **30 days a level** |
| a pet you also quiz | 2 points/day → **15 days a level** |
| one pet to Lv.12 | 165–330 days |
| **all twelve to Lv.12** | 3960 points ÷ 14 a day = **283 days ≈ 9.3 months** |

An earlier version paid per action *and* unlocked more actions each level,
which is a loop that accelerates itself: a day was worth 6 points at Lv.1 and
15 at Lv.7, so a diligent child maxed a pet in 21 days and — with no global
limit — **all twelve within the same 21 days**. The daily slot cap is what
actually controls the calendar. Level cost alone never could, because every
friendship progresses in parallel.

**Levels with no new action carry a surprise instead** (`FriendLevel.surprise`),
so no level is a dead step. A test asserts every level is one or the other.

Scoring lives in `award_pet_visit` and `record_pet_quiz`, which **decide the
point values themselves**. The function they replaced took the value as an
argument, which let a browser name its own price.

**Variety comes from events, not from bigger numbers**
(`src/lib/petEvents.ts`). One or two points a day is not what brings a child
back; the pet having something new to say is. Seasons, fixed-date festivals,
the child's birthday, each pet's own birthday, town days on the 1st and 15th,
and new-theme days all mix extra lines into whatever the child taps.
Everything is derived from the date, so a festival needs no cron and no
operator action.

Two gaps, both deliberate rather than forgotten:

- **Weather is absent.** There is no weather source in the app, and a pet
  saying 落雨啦 on a clear day is something a child in Hong Kong notices at
  once. It needs a real feed first.
- **Lunar festivals (農曆新年, 中秋) are absent.** They move every year, and a
  wrong date is worse than no greeting, so they need a supplied calendar.

**Still to come from Em:** the twelve pets' personalities and likes, which
the dialogue should be rewritten against — the current lines are generic and
interchangeable, which is exactly what the variety work is meant to avoid.
The twelve level titles are provisional too.
